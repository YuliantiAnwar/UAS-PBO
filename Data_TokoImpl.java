 ...
 * @author Acer
 */
public class Data_TokoImpl implements Toko {
    
      @Override
    public Data insert(Data data) throws SQLException {
        PreparedStatement st = koneksi_db.getConnection().prepareStatement("insert into tbl_sepatu values(?,?,?,?)");
    ...
    public void update(Data data) throws SQLException {
        PreparedStatement st = koneksi_db.getConnection().prepareStatement("update tbl_sepatu set warna=?,ukuran=?,merk=?,jumlah=?,harga=? where kode=?");
    ...
    public void delete(String kode) throws SQLException {
        PreparedStatement st = koneksi_db.getConnection().prepareStatement("delete from tbl_sepatu  where kode=?");
        st.setString(1, kode);
        st.executeUpdate();
    }
    ...
    public List getAll() throws SQLException {
        ...
        ResultSet rs = st.executeQuery("select * from tbl_sepatu");
